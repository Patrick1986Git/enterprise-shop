...
| `POST /api/v1/admin/products`; `PUT /api/v1/admin/products/{id}` | `ProductCreateDTO`: name max 255, SKU max 50, description max 5,000; price/stock/UUID constraints; each non-null image URL is max 512 Java UTF-16 code units; no repository-owned image count maximum | ADMIN | Category/product reads and writes |
...
`OrderCreateRequestDTO` and `UserCreateDTO` exist but no production controller accepts them. Their missing collection/string maxima are not current inbound endpoint surface. The product-image item bound mirrors the existing `VARCHAR(512)` persistence width conservatively: PostgreSQL measures characters, while Bean Validation applies `CharSequence.length()` and therefore counts Java UTF-16 code units. The repository contains no product, client, response-size, or deployment evidence from which to select a numeric gallery maximum, so the collection intentionally has no item-count constraint; measured production payload and rendering requirements are needed before selecting one. Generic raw-body boundedness remains a separate concern that must occur before DTO construction.

## Product gallery ordering

The administrator-supplied list order is the display order. The application persists new and replacement galleries with contiguous zero-based `sort_order` values, and JPA loads images by `sort_order` then image UUID. The first image in that order is the main image. The database guarantees non-null, non-negative positions; the application guarantees contiguous positions for its writes. It deliberately does not impose per-product uniqueness because replacement uses orphan removal and a transient delete/insert flush sequence must not make a legal reorder fail.

Migration V48 normalizes historical rows by their previous `sort_order`, with image UUID as the deterministic fallback for null or equal positions, before making the column non-null. This preserves every image but does not claim to recover an unknowable historical display order. Gaps and duplicate positions can still be read deterministically by the same `sort_order`, UUID ordering if data is written outside the application.
