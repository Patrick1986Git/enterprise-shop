...
| `OUTBOX_PROCESSING_ENABLED` | `app.outbox.processing.enabled` | defaulted | Optional; `false` leaves durable events pending and actionable without invoking handlers. | Non-secret | Boolean; default `false`. | Order worker/platform owner; enablement is a deployment choice independent of notification delivery. See [outbox architecture](../architecture/outbox-and-notifications.md). |
| `OUTBOX_PROCESSING_BATCH_SIZE` | `app.outbox.processing.batch-size` | defaulted | Optional; must be positive. | Sensitive | Integer events per polling pass; default `25`. | Order/database capacity owner; configuration-properties setter validation. See [outbox architecture](../architecture/outbox-and-notifications.md). |
| `OUTBOX_PROCESSING_FIXED_DELAY` | `app.outbox.processing.fixed-delay` | defaulted | Optional; must be positive. | Non-secret | Spring duration; default `PT10S`. | Order worker scheduling owner; configuration-properties setter validation. See [outbox architecture](../architecture/outbox-and-notifications.md). |
| `OUTBOX_PROCESSING_RETRY_DELAY` | `app.outbox.processing.retry-delay` | defaulted | Optional; must be positive. | Non-secret | Spring duration; default `PT1M`. | Order recovery owner; configuration-properties setter validation. See [outbox architecture](../architecture/outbox-and-notifications.md). |
| `OUTBOX_PROCESSING_MAX_ATTEMPTS` | `app.outbox.processing.max-attempts` | defaulted | Optional; must be positive. | Non-secret | Integer attempt count; default `3`. | Order recovery owner; configuration-properties setter validation. See [outbox architecture](../architecture/outbox-and-notifications.md). |
| `ORDER_RESERVATION_DURATION` | `app.order.reservation-expiration.duration` | defaulted | Optional; must be positive. | Non-secret | Spring duration; default `PT30M`, measured from the PostgreSQL wall-clock sample taken after checkout idempotency serialization and before cart/inventory preparation. | Order worker owner; configuration-properties setter validation. See [application lifecycle](./application-lifecycle.md). |
...
| `NOTIFICATION_DELIVERY_ENABLED` | `app.notification.delivery.enabled` | defaulted | Optional; `false` preserves pending/actionable work without delivery, while `true` requires a real configured sender and fails startup if only the no-op sender is available. | Non-secret | Boolean; default `false`. | Notification/platform owner; enable only with a configured transport. See [notification architecture](../architecture/outbox-and-notifications.md). |
| `NOTIFICATION_DELIVERY_BATCH_SIZE` | `app.notification.delivery.batch-size` | defaulted | Optional; must be positive. | Sensitive | Integer notifications per polling pass; default `25`. | Notification/database capacity owner; configuration-properties setter validation. See [notification architecture](../architecture/outbox-and-notifications.md). |
| `NOTIFICATION_DELIVERY_FIXED_DELAY` | `app.notification.delivery.fixed-delay` | defaulted | Optional; must be positive. | Non-secret | Spring duration; default `PT10S`. | Notification worker scheduling owner; configuration-properties setter validation. See [notification architecture](../architecture/outbox-and-notifications.md). |
| `NOTIFICATION_DELIVERY_MAX_ATTEMPTS` | `app.notification.delivery.max-attempts` | defaulted | Optional; must be positive. | Non-secret | Integer attempt count; default `3`. | Notification recovery owner; configuration-properties setter validation. See [notification architecture](../architecture/outbox-and-notifications.md). |
| `NOTIFICATION_DELIVERY_RETRY_DELAY` | `app.notification.delivery.retry-delay` | defaulted | Optional; must be positive. | Non-secret | Spring duration; default `PT1M`. | Notification recovery owner; configuration-properties setter validation. See [notification architecture](../architecture/outbox-and-notifications.md). |
| `NOTIFICATION_DELIVERY_CLAIM_DURATION` | `app.notification.delivery.claim-duration` | defaulted | Optional; must be positive and exceed each configured SMTP timeout when SMTP is enabled. | Non-secret | Spring duration; default `PT5M`. | Notification concurrency owner; configuration-properties and delivery-configuration validation. See [notification architecture](../architecture/outbox-and-notifications.md). |
...
## Notification delivery safety boundary

Enabled delivery fails closed unless a real `NotificationSender` is configured. In the repository-owned SMTP configuration, that means `NOTIFICATION_DELIVERY_ENABLED=true` must be paired with `NOTIFICATION_SMTP_ENABLED=true` and a non-blank `SPRING_MAIL_HOST`. The disabled worker is a valid maintenance state: notifications remain pending/actionable and are not marked `SENT`. The no-op sender is allowed only in that disabled state and throws if invoked. A durable `SENT` row means the configured transport call returned successfully; for SMTP it does not prove recipient receipt, and provider delivery remains at-least-once.

Spring relaxed binding is an implementation capability, not the canonical production contract. Every repository-supported
outbox, reservation-expiration, and notification-delivery worker control is therefore represented by an explicit
placeholder in `application-prod.yml` and a row above. The repository fallbacks preserve safe, established behavior, but
they are not recommendations for production throughput or capacity. Enablement and tuning remain deployment decisions;
an intentionally disabled worker is valid, while actionable/backlog metrics provide runtime evidence when work is
unintentionally left disabled. Invalid repository-owned worker values fail during startup, before traffic, rather than
changing persistent-state transition semantics.

## Conditional framework-bound SMTP configuration

The canonical inventory above remains limited to `${...}` placeholders explicitly present in
