...
The proposal workflow is intentionally separate from ordinary CI. It is triggered through `workflow_run` only for a successful in-repository `push` run of `CI` on `master`; the workflow definition and proposal script come from the protected default branch, not from the triggering revision or a pull request. It rechecks that the candidate is still the live `master` head before preparation, before commit, and before its lease-protected branch update. Its one job alone receives `actions: write`, `contents: write`, and `pull-requests: write`: those permissions create/update the dedicated non-protected branch and PR and explicitly dispatch the existing CI and CodeQL workflows on the proposal head. Explicit dispatch is required because events created with `GITHUB_TOKEN` do not recursively trigger workflows. Ordinary `pull_request` CI remains read-only, all checkouts keep persisted credentials disabled, no `pull_request_target` workflow is used, and no PAT or long-lived secret is introduced.
