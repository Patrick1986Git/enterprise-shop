...
public record ProductSearchCriteria(
        @Schema(
                description = "Full-text search term matched against product content.",
                example = "keyboard",
        ...
        BigDecimal maxPrice,
        @Schema(
                description = "Minimum average rating filter on a 0 to 5 scale.",
                example = "4.0",
                minimum = "0",
                maximum = "5",
                requiredMode = Schema.RequiredMode.NOT_REQUIRED
        )
        @Min(value = 0, message = "{validation.product.rating.min}")
        @Max(value = 5, message = "{validation.product.rating.max}")
        Double minRating
) {}
