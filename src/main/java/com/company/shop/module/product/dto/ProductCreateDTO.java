...
public class ProductCreateDTO {
	...
	@Schema(description = "Product display name.", example = "Wireless Keyboard", maxLength = 255, requiredMode = Schema.RequiredMode.REQUIRED)
	@NotBlank(message = "{validation.product.name.required}")
	@Size(max = 255, message = "{validation.product.name.size}")
	private String name;

	@Schema(description = "Unique stock keeping unit.", example = "KEYBOARD-001", maxLength = 50, requiredMode = Schema.RequiredMode.REQUIRED)
	@NotBlank(message = "{validation.product.sku.required}")
	@Size(max = 50, message = "{validation.product.sku.size}")
	private String sku;

	@Schema(description = "Product description.", example = "Compact wireless keyboard with rechargeable battery.", maxLength = 5000, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
	@Size(max = 5000, message = "{validation.product.description.size}")
	private String description;

	@Schema(description = "Unit price of the product.", example = "79.99", minimum = "0.01", requiredMode = Schema.RequiredMode.REQUIRED)
	@NotNull(message = "{validation.product.price.required}")
	@DecimalMin(value = "0.01", message = "{validation.product.price.min}")
	private BigDecimal price;

	@Schema(description = "Available stock quantity.", example = "25", minimum = "0", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
	@PositiveOrZero(message = "{validation.product.stock.positiveOrZero}")
	private int stock;

	@Schema(description = "Identifier of the category assigned to the product.", example = "11111111-1111-1111-1111-111111111111", requiredMode = Schema.RequiredMode.REQUIRED)
