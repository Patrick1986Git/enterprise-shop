...
-- Deterministic, obviously synthetic V48 recovery markers. This is operational fixture data,
-- not an application bootstrap path, never contains production-derived values, and intentionally
-- omits the V49 author_name and V50 credential_version columns.
INSERT INTO users (id, email, password, first_name, last_name)
VALUES
  ('10000000-0000-0000-0000-000000000001', 'restore-rehearsal@example.invalid',
   '$synthetic-not-a-login-secret$', 'Alex', 'Morgan'),
  ('10000000-0000-0000-0000-000000000002', 'restore-anonymous@example.invalid',
   '$synthetic-not-a-login-secret$', NULL, NULL);

INSERT INTO user_roles (user_id, role_id)
SELECT users.id, roles.id FROM users CROSS JOIN roles
WHERE users.id IN ('10000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000002')
  AND roles.name = 'ROLE_USER';
...
INSERT INTO products (id, sku, slug, name, description, price, stock, category_id, version)
VALUES ('30000000-0000-0000-0000-000000000002', 'SYNTHETIC-ANONYMOUS-SKU',
        'synthetic-anonymous-product', 'Synthetic anonymous product', 'V49 fallback marker',
        10.00, 9, '20000000-0000-0000-0000-000000000001', 2);

INSERT INTO product_reviews (id, product_id, user_id, rating, comment, version)
VALUES
  ('32000000-0000-0000-0000-000000000001', '30000000-0000-0000-0000-000000000001',
   '10000000-0000-0000-0000-000000000001', 5, 'named snapshot marker', 3),
  ('32000000-0000-0000-0000-000000000002', '30000000-0000-0000-0000-000000000002',
   '10000000-0000-0000-0000-000000000002', 4, 'anonymous fallback marker', 2);
